...
function open(userDataDir) {
  ...
  try { db = new Database(path.join(userDataDir, "medicore.db")); db.pragma("journal_mode = WAL"); migrate(); return db; }
  catch (e) { console.error("[db] open failed", e); db = null; return null; }
}
...
function cols(table) { return db.prepare(`PRAGMA table_info(${table})`).all().map((c) => c.name).filter((n) => n !== "rowid_"); }
function upsert(table, row) {
  ...
  const payload = {}; for (const k of keys) payload[k] = row[k] === undefined ? null : row[k]; db.prepare(sql).run(payload); return row.id ?? null;
}
...
const medicines = crud("medicines"); const customers = crud("customers"); const suppliers = crud("suppliers"); const categories = crud("categories"); const stockAdjustments = crud("stock_adjustments");
const sales = {
  ...
  save: (sale) => { if (!available()) return null; const tx = db.transaction(() => { upsert("sales", { ...sale, payments_json: JSON.stringify(sale.payments ?? []), reprints_json: JSON.stringify(sale.reprints ?? []) }); db.prepare("DELETE FROM sale_items WHERE saleId = ?").run(sale.id); const ins = db.prepare("INSERT INTO sale_items (saleId, medicineId, quantity, salePrice, discountPercent, costPriceAtSale) VALUES (?,?,?,?,?,?)"); for (const l of sale.items ?? []) { const cost = l.costPriceAtSale == null ? null : Number(l.costPriceAtSale); ins.run(sale.id, l.medicineId, l.quantity, l.salePrice, l.discountPercent ?? 0, Number.isFinite(cost) ? cost : null); } }); tx(); return sale.id; },
...
const purchases = {
  ...
  save: (purchase) => { if (!available()) return null; const tx = db.transaction(() => { upsert("purchases", purchase); db.prepare("DELETE FROM purchase_items WHERE purchaseId = ?").run(purchase.id); const ins = db.prepare("INSERT INTO purchase_items (purchaseId, medicineId, quantity, purchasePrice, batchNumber, expiryDate) VALUES (?,?,?,?,?,?)"); for (const l of purchase.items ?? []) ins.run(purchase.id, l.medicineId, l.quantity, l.purchasePrice, l.batchNumber ?? null, l.expiryDate ?? null); }); tx(); return purchase.id; },
  remove: (id) => removeOne("purchases", id),
};
const settings = {
  get: () => { if (!available()) return null; const row = db.prepare("SELECT * FROM settings WHERE id = 1").get(); if (!row) return null; return { meta: JSON.parse(row.meta_json || "{}"), settings: JSON.parse(row.settings_json || "{}") }; },
  save: (meta, s) => { if (!available()) return false; db.prepare(`INSERT INTO settings (id, meta_json, settings_json) VALUES (1, @m, @s) ON CONFLICT(id) DO UPDATE SET meta_json = @m, settings_json = @s`).run({ m: JSON.stringify(meta ?? {}), s: JSON.stringify(s ?? {}) }); return true; },
};
const auditLog = {
  add: (entry) => { if (!available()) return false; db.prepare(`INSERT INTO audit_log (id, entityType, entityId, action, username, userId, medicineName, quantity, price, timestamp) VALUES (@id,@entityType,@entityId,@action,@username,@userId,@medicineName,@quantity,@price,@timestamp)`).run({ id: entry.id, entityType: entry.entityType, entityId: entry.entityId, action: entry.action, username: entry.username ?? null, userId: entry.userId ?? null, medicineName: entry.medicineName ?? null, quantity: entry.quantity ?? null, price: entry.price ?? null, timestamp: entry.timestamp }); return true; },
  ...
  since: (isoTimestamp) => available() ? db.prepare("SELECT * FROM audit_log WHERE timestamp > ? ORDER BY timestamp ASC").all(isoTimestamp) : [],
};
...
function saveProject(data) { if (!available() || !data) return false; const tx = db.transaction(() => { for (const t of ["sale_items","sales","purchase_items","purchases","medicines","customers","suppliers","categories","stock_adjustments"]) db.prepare(`DELETE FROM ${t}`).run(); settings.save(data.meta, data.settings); for (const r of data.categories ?? []) categories.save(r); for (const r of data.suppliers ?? []) suppliers.save(r); for (const r of data.customers ?? []) customers.save(r); for (const r of data.medicines ?? []) medicines.save(r); for (const r of data.purchases ?? []) purchases.save(r); for (const r of data.sales ?? []) sales.save(r); for (const r of data.stockAdjustments ?? []) stockAdjustments.save(r); }); tx(); return true; }
function clearProject() { if (!available()) return false; const tx = db.transaction(() => { for (const t of ["sale_items","sales","purchase_items","purchases","medicines","customers","suppliers","categories","stock_adjustments","settings"]) db.prepare(`DELETE FROM ${t}`).run(); }); tx(); return true; }
module.exports = { open, available, medicines, sales, purchases, customers, suppliers, categories, stockAdjustments, settings, auditLog, loadProject, saveProject, clearProject };
