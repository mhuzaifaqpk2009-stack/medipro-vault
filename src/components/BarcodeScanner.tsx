...
type Props = { open: boolean; onClose: () => void; onDetected: (result: ScanResult) => void; title?: string };

export function BarcodeScanner({ open, onClose, onDetected, title = "Scan barcode or QR code" }: Props) {
      ...
      try {
        ...
        const tick = async () => {
          ...
          try {
            ...
            if (value) { stop(); onDetected({ value, format: first?.format }); return; }
          } catch { /* keep scanning */ }
  ...
  return <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
    <DialogContent className="max-w-xl">
      ...
      <div className="space-y-3">
        ...
        <p className="text-center text-xs text-muted-foreground">Point the camera at a barcode or QR code. The code is detected automatically.</p>
