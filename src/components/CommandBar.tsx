...
export function CommandBar() {
  ...
  const results = useMemo(() => { const q=query.trim().toLowerCase(); const list=!q?commands:commands.filter((c)=>`${c.label} ${c.description} ${c.shortcut} ${"keywords" in c ? c.keywords : ""}`.toLowerCase().includes(q)); return list.slice(0,50); }, [commands, query]);
  useEffect(() => { setSelected(0); resultRefs.current = []; }, [query]);
  useEffect(() => { const node = resultRefs.current[selected]; node?.scrollIntoView({ block: "nearest" }); }, [selected, results.length, open]);
