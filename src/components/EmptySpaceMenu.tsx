...
export function EmptySpaceMenu(){ const pathname=useRouterState({select:s=>s.location.pathname});const navigate=useNavigate();const data=useProjectStore(s=>s.data);const user=useSession(s=>s.user);const[open,setOpen]=useState(false);const[point,setPoint]=useState({x:0,y:0});useEffect(()=>{const main=document.querySelector("main");if(!main)return;const onContext=(e:MouseEvent)=>{if(isInteractive(e.target))return;e.preventDefault();setPoint({x:e.clientX,y:e.clientY});setOpen(true);};const close=()=>setOpen(false);main.addEventListener("contextmenu",onContext);window.addEventListener("click",close);window.addEventListener("resize",close);return()=>{main.removeEventListener("contextmenu",onContext);window.removeEventListener("click",close);window.removeEventListener("resize",close);};},[pathname]);const item=NAV.find(n=>n.to===pathname);const label=item?navLabel(item,data?.settings):pathname;const add=ADD_BY_PATH[pathname];const canPin=!!user&&(user.role==="admin"||user.permissions.pinPanel===true);const left=Math.min(point.x,Math.max(8,window.innerWidth-240));const top=Math.min(point.y,Math.max(8,window.innerHeight-130));if(!open||!data||!item)return null;const triggerAdd=()=>{setOpen(false);if(add?.quick){runQuickAction(add.quick,navigate);return;}const buttons=Array.from(document.querySelectorAll<HTMLElement>("main button")).filter(b=>{const text=(b.textContent||"").replace(/\s+/g," ").trim();return add?.patterns.some(r=>r.test(text));});if(buttons[0]){buttons[0].click();return;}if(pathname==="/app/messages")navigate({to:pathname});};const pin=()=>{setOpen(false);togglePin({id:pathname,label,kind:"nav",to:pathname});};return <div className="fixed inset-0 z-[10000] pointer-events-none"><div className="pointer-events-auto absolute min-w-52 rounded-md border bg-popover p-1 text-popover-foreground shadow-elevated" style={{left,top}} onClick={e=>e.stopPropagation()}><div className="px-2 py-1 text-[11px] uppercase tracking-widest text-muted-foreground">{label}</div>{add&&<button className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-sm hover:bg-accent" onClick={triggerAdd}><Plus className="h-4 w-4"/>{add.label}</button>}{canPin&&<button className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-sm hover:bg-accent" onClick={pin}><Pin className="h-4 w-4"/>Pin current tab</button>}{!add&&!canPin&&<p className="px-2 py-1.5 text-xs text-muted-foreground">No quick action available here.</p>}</div></div>; }
