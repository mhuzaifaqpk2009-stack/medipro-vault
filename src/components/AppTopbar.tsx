...
const HELP_SECTIONS = [
  ...
  { title: "Sales / POS", items: [
    ...
    ["Load a prescription", "In Sell, choose Prescription, search the patient, and load the prescription. Its medicines and quantities are added to the cart. Loading a scheduled visit advances the next visit by the configured repeat interval."],
  ...
  { title: "Prescriptions", items: [
    ["Create a prescription", "Enter patient name, phone, doctor and diagnosis, then add medicines with quantities and instructions."],
    ["Follow-up schedule", "Set a next visit date, repeat every day/week/month/year (including custom values), and choose how many days before the visit notifications should appear."],
    ["Prescription privacy", "A prescription can be Admin only, visible to all users with prescription access, or restricted to selected users. Admin always retains access."],
    ["Stock readiness", "The prescription detail view shows current medicine price and stock. Missing or insufficient stock is highlighted in red."],
    ["Edit / print / delete", "These actions are separately permission-controlled under Admin Settings → Users."],
  ]},
  ...
  { title: "Reports & Notifications", items: [
    ...
    ["Notifications", "Admins and users with View notifications permission can receive enabled pharmacy activity and prescription follow-up notifications."],
  ]},
  ...
  { title: "Settings & Tips", items: [
    ...
    ["Sign out", "Sign out returns to the login screen without deleting the user account or pharmacy project. Ctrl+Esc is also available as a logout shortcut."],
  ]},
] as const;
