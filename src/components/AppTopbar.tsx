...
export function AppTopbar() {
  ...
  return (
    <div className="sticky top-0 z-30">
      <header
        className="flex items-center gap-2 border-b bg-background/85 px-3 backdrop-blur"
        style={{ height }}
      >
        <SidebarTrigger />
        <Separator orientation="vertical" className="mx-1 h-6" />
        <div className="flex min-w-0 items-center gap-2">
          <img src="./logo.png" alt="" className="h-6 w-6 object-contain" draggable={false} />
          ...
          {isAdmin && (
            <>
              <Button
                variant="ghost"
                size="sm"
                {...pinContext({ id: "cmd:save", label: "Save", kind: "cmd" })}
                onClick={async () => {
                  const ok = await save();
                  if (ok) toast.success("Saved");
                }}
                title="Save (Ctrl+S) · right-click to pin"
              >
              ...
              <Button
                variant="ghost"
                size="sm"
                {...pinContext({ id: "cmd:backup", label: "Backup", kind: "cmd" })}
                onClick={async () => {
                  try {
                ...
                title="Create backup (F5) · right-click to pin"
              >
                <HardDriveDownload className="mr-1.5 h-4 w-4" /> Backup
              </Button>
              <Separator orientation="vertical" className="mx-1 h-6" />
            </>
          )}
      ...
      <ResizeHandle
        ...
        min={44}
        max={96}
        onChange={(v) => mutate((d) => { d.settings.topbarHeight = v; })}
      />
    </div>
  );
}
