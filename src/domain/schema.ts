...
export interface Medicine {
  id: string; name: string; genericName?: string; company?: string; categoryId?: string;
  batchNumber?: string; barcode?: string; purchasePrice: number; salePrice: number; mrp: number;
  stockQuantity: number; minimumStock: number; expiryDate?: string; manufactureDate?: string;
  rackNumber?: string; supplierId?: string; description?: string; pinOrder?: "first" | "last";
}
...
export interface SaleItem {
  ...
  /** Acquisition cost captured when the item is sold; never changes with later medicine edits. */
  costPriceAtSale?: number;
}
export interface SalePayment { method: "cash" | "card" | "online"; amount: number; reference?: string; }
export interface Sale {
  id: string; invoiceNumber: string; date: string; customerId?: string; remark?: string;
  items: SaleItem[]; discount: number; taxPercent: number; payments: SalePayment[];
  status: "completed" | "cancelled" | "returned"; notes?: string; createdBy?: string; reprints?: string[];
}
...
export function createEmptyProject(name: string, passwordProtected: boolean): ProjectData {
  ...
  return {
    ...
    settings: {
      ...
      autoSaveEnabled: true, autoBackupEnabled: false, autoBackupIntervalHours: 24,
      autoSaveIntervalMinutes: 5, theme: "light", passwordProtected,
    },
