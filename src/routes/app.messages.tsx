...
function MessagesPage() {
  ...
  return <div className="mx-auto max-w-5xl p-6 md:p-10">
    ...
    <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
      <section className="surface-card p-5"><h2 className="font-display text-base font-semibold">New message</h2><p className="mt-1 text-xs text-muted-foreground">{canMessageUsers ? "Managers can message staff; Admins can message anyone." : "You can always send a message to an administrator."}</p><div className="mt-5 space-y-4"><div><Label>To</Label><Select value={recipientId} onValueChange={setRecipientId}><SelectTrigger className="mt-1.5"><SelectValue placeholder="Select recipient" /></SelectTrigger><SelectContent>{recipients.map((r) => <SelectItem key={r.id} value={r.id}>{r.username}{r.role === "admin" ? " (Admin)" : r.roleTemplate === "manager" ? " (Manager)" : ""}</SelectItem>)}</SelectContent></div><div><Label>Priority</Label><Select value={priority} onValueChange={(v) => setPriority(v as MessagePriority)}><SelectTrigger className="mt-1.5"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="normal">🟢 Normal</SelectItem><SelectItem value="important">🟡 Important</SelectItem><SelectItem value="urgent">🔴 Urgent</SelectItem></SelectContent></div><div><Label>Message</Label><Textarea value={body} onChange={(e) => setBody(e.target.value)} onKeyDown={(e) => { if ((e.ctrlKey || e.metaKey) && e.key === "Enter") { e.preventDefault(); void send(); } }} placeholder="Type an instruction…" className="mt-1.5 min-h-32" maxLength={1000} /><p className="mt-1 text-right text-[11px] text-muted-foreground">Ctrl+Enter to send · {body.length}/1000</p></div><Button className="w-full" disabled={!body.trim() || !recipientId} onClick={() => void send()}><Send className="mr-2 h-4 w-4" /> Send message</Button></div></section>
