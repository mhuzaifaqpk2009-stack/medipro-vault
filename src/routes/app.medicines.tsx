...
const empty = (): Medicine => ({ id: "", name: "", genericName: "", company: "", batchNumber: "", barcode: "", purchasePrice: 0, salePrice: 0, mrp: 0, stockQuantity: 0, minimumStock: 5, expiryDate: "", rackNumber: "" });
...
function MedicinesPage() {
  ...
  const [searchBy, setSearchBy] = useState<MedicineSearchBy>(() => {
    const saved = useProjectStore.getState().data?.settings.defaultSearchBy;
    return saved === "generic" || saved === "company" ? saved : "name";
  });
  ...
  function save(m: Medicine, keepOpen=false) { if(!m.name.trim()){toast.error("Name is required");return;} if(!m.id){const key=m.name.trim().toLowerCase();const existing=meds.find((x)=>x.name.trim().toLowerCase()===key);if(existing){setDupe({m,existingId:existing.id,keepOpen});return;}} commit(m,keepOpen); }
  ...
  return <div className="mx-auto max-w-7xl p-6 md:p-8">
    ...
    <div className="surface-card overflow-hidden"><Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Batch</TableHead><TableHead>Barcode</TableHead><TableHead className="text-right">Stock</TableHead><TableHead className="text-right">Sale</TableHead><TableHead>Expiry</TableHead><TableHead className="w-24"></TableHead></TableRow></TableHeader><TableBody>{sorted.length===0&&<TableRow><TableCell colSpan={7} className="py-14 text-center text-sm text-muted-foreground">No medicines. Add your first SKU.</TableCell></TableRow>}{pageItems.map((m)=><MedicineRow key={m.id} m={m} sym={sym} canSeeHistory={canSeeHistory} canEdit={canEdit} canDelete={canDelete} canPin={canPin} onEdit={()=>{if(!canEdit)return;setFormKey((k)=>k+1);setEditing(m);}} onDelete={()=>remove(m.id)} onHistory={()=>setHistoryFor(m)} onPin={(pin)=>setPin(m.id,pin)} />)}</TableBody></Table></div>
...
function MedicineEditor({value,onCancel,onSave}:{value:Medicine|null;onCancel:()=>void;onSave:(m:Medicine,keepOpen?:boolean)=>void}){
  const categories=useProjectStore((s)=>s.data!.categories); const actionHotkeys=useProjectStore((s)=>s.data!.settings.actionHotkeys); const [m,setM]=useState<Medicine|null>(value); const cur=m??value??empty(); const quickCombo=effectiveActionHotkey("quick-add-medicine",actionHotkeys);
  ...
  return <Dialog open onOpenChange={(o)=>!o&&onCancel()}><DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto"><DialogHeader><DialogTitle>{value.id?"Edit medicine":"New medicine"}</DialogTitle></DialogHeader><div className="grid gap-3 sm:grid-cols-2"><F label="Name *"><Input value={cur.name} onChange={(e)=>upd("name",e.target.value)} autoFocus /></F><F label="Generic name"><Input value={cur.genericName??""} onChange={(e)=>upd("genericName",e.target.value)} /></F><F label="Company"><Input value={cur.company??""} onChange={(e)=>upd("company",e.target.value)} /></F><F label="Category (optional)"><Select value={cur.categoryId||"none"} onValueChange={(v)=>upd("categoryId",v==="none"?"":v)}><SelectTrigger><SelectValue placeholder="Uncategorised" /></SelectTrigger><SelectContent><SelectItem value="none">Uncategorised</SelectItem>{categories.map((c)=><SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent></Select></F><F label="Barcode"><Input value={cur.barcode??""} onChange={(e)=>upd("barcode",e.target.value)} /></F><F label="Batch #"><Input value={cur.batchNumber??""} onChange={(e)=>upd("batchNumber",e.target.value)} /></F><F label="Rack"><Input value={cur.rackNumber??""} onChange={(e)=>upd("rackNumber",e.target.value)} /></F><F label="Purchase price"><Input type="number" value={cur.purchasePrice||""} onChange={(e)=>upd("purchasePrice",+e.target.value||0)} /></F><F label="Sale price"><Input type="number" value={cur.salePrice||""} onChange={(e)=>upd("salePrice",+e.target.value||0)} /></F><F label="MRP"><Input type="number" value={cur.mrp||""} onChange={(e)=>upd("mrp",+e.target.value||0)} /></F><F label="Stock"><Input type="number" value={cur.stockQuantity||""} onChange={(e)=>upd("stockQuantity",+e.target.value||0)} /></F><F label="Minimum stock"><Input type="number" value={cur.minimumStock||""} onChange={(e)=>upd("minimumStock",+e.target.value||0)} /></F><F label="Expiry (YYYY-MM-DD)"><Input type="date" value={cur.expiryDate??""} onChange={(e)=>upd("expiryDate",e.target.value)} /></F></div><DialogFooter><Button variant="ghost" onClick={onCancel}>Cancel</Button>{!value.id&&<Button variant="secondary" onClick={()=>onSave(cur,true)}>Quick Add{quickCombo?` (${quickCombo})`:""}</Button>}<Button onClick={()=>onSave(cur)}>Save</Button></DialogFooter></DialogContent></Dialog>;
}
