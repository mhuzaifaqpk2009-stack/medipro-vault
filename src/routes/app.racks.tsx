...
function RacksPage(){
  const data=useProjectStore(s=>s.data)!; const mutate=useProjectStore(s=>s.mutate); const racks=data.settings.racks ?? []; const user=currentUser(); const canAddRack=user?.role==="admin"||user?.roleTemplate==="manager"||user?.permissions.racksAdd===true;
