...
export function syncInventoryBatches(data: ProjectData): void {
  ...
  for(const purchase of data.purchases) purchase.items.forEach((item,index)=>{
    ...
    if(existing){
      ...
      existing.initialQuantity=Math.max(0,item.quantity);
      existing.quantity=Math.max(0,item.quantity-previouslySold);
      existing.purchasePrice=item.purchasePrice;
      existing.batchNumber=item.batchNumber;
      existing.expiryDate=item.expiryDate;
      existing.receivedDate=purchase.receivedDate??purchase.purchaseDate;
  ...
  // Deleted purchases remain auditable but can never contribute sellable stock.
...
/** Recalculate medicine aggregate stock from non-negative batch quantities. */
export function reconcileMedicineStock(data: ProjectData): void {
  const batches=data.settings.inventoryBatches??[];
  for(const medicine of data.medicines){
    medicine.stockQuantity=batches.filter(b=>b.medicineId===medicine.id).reduce((sum,b)=>sum+Math.max(0,b.quantity),0);
  }
}
...
/** Allocate stock FEFO: earliest valid expiry first, then FIFO by received date. Expired lots are never sold. */
