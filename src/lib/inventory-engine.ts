...
export function syncInventoryBatches(data: ProjectData): void {
  ...
  // One-time migration: old databases only know aggregate medicine stock.
  // Keep that quantity in a legacy lot and mark historical purchases as
  // metadata-only lots (zero available quantity). This avoids inventing
  // historical batch quantities that were already consumed by old sales.
  if ((data.settings.inventoryBatchMigrationVersion ?? 0) < 1) {
    for (const medicine of data.medicines) {
      if (!batches.some((b) => b.sourceKey === `legacy:${medicine.id}`)) {
  ...
  // keys are left untouched so a sale can never recreate consumed stock.
  for (const purchase of data.purchases) purchase.items.forEach((item, index) => {
    const sourceKey = `purchase:${purchase.id}:${index}`;
