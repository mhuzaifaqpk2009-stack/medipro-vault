...
export function syncInventoryBatches(data: ProjectData): void {
  ...
  // A deleted purchase no longer contributes stock. Keep its batch record for
  // audit/history, but make its available quantity zero.
  for(const batch of batches) if(batch.sourceKey?.startsWith("purchase:")&&!livePurchaseKeys.has(batch.sourceKey)) batch.quantity=0;

  for(const medicine of data.medicines){const lots=batches.filter(b=>b.medicineId===medicine.id);medicine.stockQuantity=lots.reduce((sum,b)=>sum+Math.max(0,b.quantity),0);}
}
