...
export function ItemMenuHost() {
  ...
  useEffect(() => { openFn = (t, p) => { setTarget(t); setAt(p); setRenaming(false); setName(settingsRef.current?.tabRenames?.[t.id] ?? ""); }; return () => { openFn = null; }; }, []);
  ...
  const isNav = target.kind === "nav" && !!target.to && NAV.some((n) => n.to === target.to);
  function close() { setTarget(null); setRenaming(false); }
  function doRename() { renameTab(target!.id, name); toast.success("Renamed"); close(); }
  function move(direction: "forward" | "backward" | "top" | "bottom") { if (!isAdmin || !target.to) return; moveNavItem(target.to, direction); toast.success(direction === "top" ? "Moved to top" : direction === "bottom" ? "Moved to bottom" : direction === "forward" ? "Moved forward" : "Moved backward"); close(); }
  function addToMacro() { if (!target.to) return; sessionStorage.setItem("medipro:macro-prefill-route", target.to); window.location.href = "/app/macros"; }
  return <div className="fixed inset-0 z-[100]" onMouseDown={() => close()} onContextMenu={(e) => { e.preventDefault(); close(); }}><div role="menu" style={{ left, top, width: MENU_W }} onMouseDown={(e) => e.stopPropagation()} className="absolute rounded-md border bg-popover p-1 text-popover-foreground shadow-elevated">
    ...
    {target.to && <button className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-left text-sm hover:bg-accent" onClick={() => { window.location.href = target.to!; }}><ExternalLink className="h-4 w-4" /> Open</button>}
