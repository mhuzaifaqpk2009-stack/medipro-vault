...
export type MenuTarget = PinnedItem & { canRename?: boolean; element?: HTMLElement; tabKey?: string };
...
export function togglePin(item: PinnedItem) { const state = useProjectStore.getState(); const user = useSession.getState().user; if (!user || (user.role !== "admin" && user.permissions.pinPanel !== true)) { toast.error("You do not have permission to pin items"); return false; } let pinned = false; state.mutate((d) => { const list = d.settings.pinnedItems ?? []; if (list.some((p) => p.id === item.id)) d.settings.pinnedItems = list.filter((p) => p.id !== item.id); else { d.settings.pinnedItems = [...list, { id: item.id, label: item.label, kind: item.kind, to: item.to }]; pinned = true; } }); return pinned; }
...
export function renameTab(path: string, name: string) { useProjectStore.getState().mutate((d) => { const map = { ...(d.settings.tabRenames ?? {}) }; if (name.trim()) map[path] = name.trim(); else delete map[path]; d.settings.tabRenames = map; }); }
export function moveNavItem(path: string, direction: "forward" | "backward" | "top" | "bottom") { useProjectStore.getState().mutate((d) => { const order = orderNav(d.settings.tabOrder).map((n) => n.to); const index = order.indexOf(path); if (index < 0) return; order.splice(index, 1); if (direction === "top") order.unshift(path); else if (direction === "bottom") order.push(path); else if (direction === "forward") order.splice(Math.min(index + 1, order.length), 0, path); else order.splice(Math.max(index - 1, 0), 0, path); d.settings.tabOrder = order; }); }
...
const MENU_W = 246;
function tabRenameKey(pathname: string, value: string) { return `tab:${pathname}:${value}`; }
function tabLabelNodes(el: HTMLElement) { return Array.from(el.childNodes).filter((n) => n.nodeType === Node.TEXT_NODE) as Text[]; }
function applyTabLabel(el: HTMLElement, label: string) { const nodes = tabLabelNodes(el); if (nodes.length) nodes[nodes.length - 1].nodeValue = label; else el.setAttribute("aria-label", label); el.dataset.renamedTab = label; }
