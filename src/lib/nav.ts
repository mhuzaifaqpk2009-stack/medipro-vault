/**
 * Single source of truth for the workspace navigation model.
 * The sidebar renders from this list, and tab shortcuts (Ctrl+N, Ctrl+Tab)
 * follow exactly the same visual order so reordering tabs re-maps shortcuts.
 */
import {
  LayoutDashboard, Pill, ShoppingCart, Truck, Users, Building2, Tags, Boxes,
  BarChart3, Settings, Receipt, Calculator,
} from "lucide-react";
...
export function groupLabel(group: string, settings?: PharmacySettings) {
  return settings?.groupRenames?.[group] || group;
}

export function navLabel(item: NavItem, settings?: PharmacySettings) {
...
export function visibleNavItems(settings?: PharmacySettings, user?: StoredUser | null): NavItem[] {
  ...
  const allowed = orderNav(settings?.tabOrder).filter((i) => {
    if (i.to === "/app/calculator" && settings?.calculatorEnabled === false) return false;
    if (i.adminOnly) return isAdmin;
    if (i.perm) return isAdmin || !!user?.permissions[i.perm];
    return true;
  });
  return GROUPS.flatMap((g) => allowed.filter((i) => groupOf(i) === g));
}
